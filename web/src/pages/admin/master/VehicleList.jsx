...
export default function VehicleFleetDirectory() {
  ...
  const getDriverName = (vehicle) => {
    ...
    if (vehicle.driver?.name) return vehicle.driver.name;
    if (vehicle.driver_id) {
      const driver = drivers.find(d => d.id === vehicle.driver_id);
      if (driver) {
        // Try different possible name fields
        return driver.name || driver.driver_name || driver.full_name || "Unassigned";
      }
      return "Unassigned";
    }
    return "Unassigned";
  ...
  // Helper function to get driver display name for dropdown
  const getDriverDisplayName = (driver) => {
    // Try to get name from various possible fields
    const name = driver.name || driver.driver_name || driver.full_name || "Unknown";
    // If phone is available and name is "Unknown" or empty, show phone
    if ((!name || name === "Unknown") && driver.phone) {
      return driver.phone;
    }
    // Show name with phone in parentheses if available
