...
const normalized = data.map((city) => ({
  ...
  createdAt: city.created_at
    ...
    console.log("📡 Normalized cities:", normalized);
...
const handleCreateCity = async () => {
  ...
  try {
    ...
    if (!response.data) {
      alert("Failed to create city: No response from server");
      return;
    }

    // Check if city was created successfully
    if (!response.data.success) {
      alert("Failed to create city: " + (response.data.message || "Unknown error"));
      return;
    }

    // Get the created city data - handle both cases
    let created = response.data.data;
    
    // If data is null, create a fallback object
    if (!created) {
      console.warn("⚠️ Response data is null, using fallback");
      created = {
    ...
    console.log("📡 Created city object:", created);
              ...
              {loading ? (
                ...
                filteredCities.map((item) => (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-5 py-4">{item.pincode}</td>
                    <td className="px-5 py-4">
  ...
  />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              ...
              <button onClick={() => setViewOpen(false)} className="rounded-lg p-1 hover:bg-gray-100">
                <X size={22} />
              </button>
            </div>
              ...
              <div>
                ...
                <p className="text-lg font-semibold">
                  <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 ${
                    selectedCity.status === "active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                  }`}>
                    <span className={`h-2 w-2 rounded-full ${selectedCity.status === "active" ? "bg-green-500" : "bg-red-500"}`} />
            ...
            <div className="flex justify-end border-t px-6 py-4">
              <button onClick={() => setViewOpen(false)} className="rounded-lg bg-blue-600 px-5 py-2 text-white hover:bg-blue-700">
                Close
              </button>
            </div>
          </div>
      ...
      {editOpen && selectedCity && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-xl font-semibold">✏️ Edit City #{selectedCity.id}</h2>
              <button
                onClick={() => { setEditOpen(false); resetForm(); setSelectedCity(null); }}
                className="rounded-lg p-1 hover:bg-gray-100"
              >
                <X size={22} />
              </button>
