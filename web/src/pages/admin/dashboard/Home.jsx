...
export default function Shipments() {
  ...
  const delayedCount = shipments.filter(
    (s) => s.status === "delayed" || s.status === "Delayed",
  ).length;
  ...
  const filteredShipments = shipments.filter((shipment) => {
    ...
    const matchesStatus =
      ...
      (statusFilter === "in-transit" &&
        (shipment.status === "In Transit" || shipment.status === "transit")) ||
  ...
  const exportCurrentViewCSV = () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `shipments_${new Date().toISOString().split("T")[0]}.csv`,
      );
  ...
  const exportCurrentViewExcel = () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `shipments_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const exportCurrentViewPDF = () => {
    ...
    try {
      ...
      doc.text(
        `Filter: ${statusFilter !== "all" ? statusFilter : "All"} | Search: ${searchTerm || "None"}`,
        pageWidth / 2,
        34,
        { align: "center" },
      );
  ...
  const viewChallan = (shipment) => {
    window.open(
      `${API_BASE}/shipments/${shipment.id}/challan-preview`,
      "_blank",
    );
  };
            ...
            <ViewModalBody>
              <ViewGrid>
                ...
                <ViewItem fullWidth>
                  <label>📝 Notes</label>
                  <p className="notes">
                    {selectedShipment.notes || "No notes"}
                  </p>
                </ViewItem>
                <ViewItem fullWidth>
                  <label>📄 Challan Number</label>
                  <span className="challan">
                    {selectedShipment.challan_number || "Not generated"}
                  </span>
                </ViewItem>
              </ViewGrid>
