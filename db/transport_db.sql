...
-- =============================================
-- Runtime tables used by the payment, activity-log, and challan APIs
-- =============================================
...
BEGIN
    CREATE TABLE dbo.payments (
        ...
        [checkpoint] NVARCHAR(255) NULL,
        upi_id NVARCHAR(255) NULL,
        upi_ref NVARCHAR(255) NULL,
        note NVARCHAR(MAX) NULL,
        status NVARCHAR(20) NOT NULL CONSTRAINT DF_payments_status DEFAULT 'pending',
        paid_by NVARCHAR(255) NOT NULL CONSTRAINT DF_payments_paid_by DEFAULT 'admin',
        created_at DATETIME NOT NULL CONSTRAINT DF_payments_created_at DEFAULT GETDATE()
    );
END
GO
...
BEGIN
    CREATE TABLE dbo.system_logs (
        ...
        created_at DATETIME NOT NULL CONSTRAINT DF_system_logs_created_at DEFAULT GETDATE()
    );
END
GO

IF OBJECT_ID(N'[dbo].[challans]', N'U') IS NULL
BEGIN
    CREATE TABLE dbo.challans (
        ...
        advance_paid DECIMAL(12,2) NOT NULL CONSTRAINT DF_challans_advance_paid DEFAULT 0,
        status NVARCHAR(20) NOT NULL CONSTRAINT DF_challans_status DEFAULT 'Active',
        created_at DATETIME NOT NULL CONSTRAINT DF_challans_created_at DEFAULT GETDATE()
    );
END
GO

IF OBJECT_ID(N'[dbo].[challans]', N'U') IS NOT NULL
    AND COL_LENGTH('dbo.challans', 'advance_paid') IS NULL
BEGIN
    ALTER TABLE dbo.challans ADD advance_paid DECIMAL(12,2) NOT NULL
        CONSTRAINT DF_challans_advance_paid_existing DEFAULT 0;
END
GO
...
BEGIN
    ...
    IF COL_LENGTH('dbo.shipments', 'challan_number') IS NULL
        ALTER TABLE dbo.shipments ADD challan_number NVARCHAR(50) NULL;

    IF COL_LENGTH('dbo.shipments', 'pickup_location') IS NULL
        ALTER TABLE dbo.shipments ADD pickup_location NVARCHAR(255) NULL;

    IF COL_LENGTH('dbo.shipments', 'delivery_location') IS NULL
        ALTER TABLE dbo.shipments ADD delivery_location NVARCHAR(255) NULL;

    IF COL_LENGTH('dbo.shipments', 'goods_desc') IS NULL
        ALTER TABLE dbo.shipments ADD goods_desc NVARCHAR(MAX) NULL;

    IF COL_LENGTH('dbo.shipments', 'weight_type') IS NULL
        ALTER TABLE dbo.shipments ADD weight_type NVARCHAR(20) NULL;
END
GO
