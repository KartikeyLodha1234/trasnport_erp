...
def get_shipments():
    try:
        print("🔍 GET /api/shipments/ called")
...
def create_shipment(data: dict):
    try:
        print("=" * 60)
        print("🔍 POST /api/shipments/ called")
        print(f"📤 Data received: {data}")
        
        # ✅ Log the locations and consignor/consignee
        print(f"📍 Pickup Location: {data.get('pickup_location')}")
        print(f"📍 Delivery Location: {data.get('delivery_location')}")
        print(f"📍 Consignor ID: {data.get('consignor_id')}")
        print(f"📍 Consignee ID: {data.get('consignee_id')}")
        ...
        shipment_data = {
            ...
            'freight_charge': float(data.get('freight_charge', 0)),
            'loading_charges': float(data.get('loading_charges', 0)),
            'unloading_charges': float(data.get('unloading_charges', 0)),
        ...
        print(f"📦 Shipment data to save: {shipment_data}")
        ...
        new_shipment = db.get_shipment_by_id(shipment_id)
        print(f"✅ Shipment created: {new_shipment}")
